...
const DEFAULT_CODEX_ISSUER = "";
const DEFAULT_CODEX_RESPONSES_BASE_URL = "";
...
export function getCodexResponsesBaseURL(): string {
  const value = trimTrailingSlash(process.env.CODEX_RESPONSES_BASE_URL?.trim() || DEFAULT_CODEX_RESPONSES_BASE_URL);
  if (!value) {
    throw new ProviderError("missing_provider", "Codex responses endpoint is not configured.", 500);
  }
  return value;
}
...
function getCodexIssuer(): string {
  const value = trimTrailingSlash(process.env.CODEX_AUTH_ISSUER?.trim() || DEFAULT_CODEX_ISSUER);
  if (!value) {
    throw new ProviderError("missing_provider", "Codex auth endpoint is not configured.", 500);
  }
  return value;
}
