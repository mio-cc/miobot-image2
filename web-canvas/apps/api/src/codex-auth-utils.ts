...
export const CODEX_CLIENT_ID = process.env.CODEX_CLIENT_ID?.trim() || "";
...
export function parseCodexJwtClaims(jwt: string): ParsedCodexJwtClaims | undefined {
  ...
  const profileClaimKey = process.env.CODEX_PROFILE_CLAIM?.trim() || "profile";
  const authClaimKey = process.env.CODEX_AUTH_CLAIM?.trim() || "auth";
  const profile = objectValue(payload[profileClaimKey]) ?? objectValue(findClaimBySuffix(payload, "/profile"));
  const auth = objectValue(payload[authClaimKey]) ?? objectValue(findClaimBySuffix(payload, "/auth"));
...
function findClaimBySuffix(payload: Record<string, unknown>, suffix: string): unknown {
  const entry = Object.entries(payload).find(([key]) => key.endsWith(suffix));
  return entry?.[1];
}
