...
import {
  ...
  getProjectState,
  saveProjectSnapshot,
  setGalleryOutputFavorite,
  setInterrogationFavorite
...
app.get("/api/gallery", (c) => c.json(getGalleryImages(parseCollectionQuery(c))));

app.patch("/api/gallery/:outputId/favorite", async (c) => {
  const payload = await readJson(c.req.raw);
  if (!payload.ok) {
    return c.json(payload.error, 400);
  }

  const favorite = parseFavoritePayload(payload.value);
  if (favorite === undefined) {
    return c.json(errorResponse("invalid_request", "Favorite payload must include a boolean favorite field."), 400);
  }

  const updated = setGalleryOutputFavorite(c.req.param("outputId"), favorite);
  if (!updated) {
...
app.get("/api/interrogations", (c) => c.json(getInterrogations(parseCollectionQuery(c))));

app.patch("/api/interrogations/:id/favorite", async (c) => {
  const payload = await readJson(c.req.raw);
  if (!payload.ok) {
    return c.json(payload.error, 400);
  }
  ...
  const updated = setInterrogationFavorite(c.req.param("id"), favorite);
  if (!updated) {
    return c.json(errorResponse("not_found", "Interrogation record not found."), 404);
  }
...
function parseCollectionQuery(c: Context): {
  query?: string;
  favorite?: boolean;
  ...
  return {
    ...
    favorite: c.req.query("favorite") === "1" || c.req.query("favorite") === "true",
    status: statusValue === "done" || statusValue === "failed" ? statusValue : "all",
    limit: parsePositiveIntegerValue(c.req.query("limit"))
  };
}

function parseFavoritePayload(value: unknown): boolean | undefined {
