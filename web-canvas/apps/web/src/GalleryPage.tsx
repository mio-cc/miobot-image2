...
export function GalleryPage({ onReuse }: GalleryPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadGallery(): Promise<void> {
      ...
      try {
        const response = await fetch(apiPath("/gallery"), {
  ...
  function downloadItem(item: GalleryImageItem): void {
    window.open(apiPath(`/assets/${encodeURIComponent(item.asset.id)}/download`), "_blank", "noopener,noreferrer");
