...
- API: Hono on `http://localhost:8787` by default.
...
- Codex login cannot complete: confirm the configured auth endpoint is reachable, keep the device-login dialog open until authorization finishes, and restart the flow if the user code expires. Do not paste or log token values.
- Custom provider endpoint: set `OPENAI_BASE_URL` in `.env`, for example `<API_BASE_URL>`, then restart the API or Docker container, or enter a local Base URL in `配置`. The endpoint must be OpenAI-compatible and support the configured image model.
