...
- API：Hono，默认地址为 `http://localhost:8787`。
...
- Codex 登录无法完成：确认机器可以访问 the configured auth endpoint，保持设备登录弹窗打开直到授权完成；用户码过期后重新开始登录流程。不要粘贴或记录 token 值。
- 自定义 provider 地址：在 `.env` 中设置 `OPENAI_BASE_URL`，例如 `<API_BASE_URL>`，然后重启 API 或 Docker 容器；也可以在 `配置` 中填写本地 Base URL。该端点必须兼容 OpenAI API，并支持当前配置的图像模型。
