import { dedupeImageUrls, type NapcatSendResult } from '../../napcat/src/index.js';
...
export type ReplyKind = 'text' | 'image' | 'multiImage';
...
export interface ReplyDispatchResult extends NapcatSendResult {
  kind: ReplyKind;
  strategy: ReplyStrategy;
...
export function resolveReplyStrategy(config: ReplyStrategyConfig, kind: ReplyKind): ReplyStrategy {
  const fallback = kind === 'text' ? 'forward' : kind === 'image' ? 'forward' : 'forward';
  return normalizeStrategy(config[kind], fallback);
}
