...
export interface BotTtsConfig { enabled: boolean; provider: BotTtsProvider; apiUrl: string; apiKey: string; model: string; voiceId: string; voice: string; format: BotTtsFormat; autoTextMaxChars: number; timeoutMs: number; speed: number; volume: number; latency: BotTtsLatency }
...
export function getDefaultPrompts() { return { enhancePromptTemplate: P.enhance, templatePromptTemplate: P.fill, referencedTemplatePromptTemplate: P.refFill, templateConvertPromptTemplate: P.convert, templateTitlePromptTemplate: P.title, translationPromptTemplate: P.translate, interrogatePromptTemplate: P.interrogate, interrogateTemplatePromptTemplate: P.interrogateTpl, safeRewritePromptTemplate: P.safe, promptsChatSearchQueryPromptTemplate: P.pcQuery, promptsChatSmartPromptTemplate: P.pcSmart, freeModePromptTemplate: P.free }; }
...
function normalizeAppConfig(input: R, warnings: string[]): AppConfig {
  ...
  hf.filters.search = str(hf.filters.search, '', false).trim(); hf.filters.author = str(hf.filters.author, '', false).trim(); hf.filters.pipelineTag = str(hf.filters.pipelineTag, 'text-generation', false).trim(); hf.filters.tags = str(hf.filters.tags, '', false).trim(); hf.filters.library = str(hf.filters.library, '', false).trim(); hf.filters.inference = pick(hf.filters.inference, ['', 'warm', 'cold', 'frozen'] as const, 'warm'); hf.filters.gated = pick(hf.filters.gated, ['', 'false', 'true'] as const, 'false'); hf.filters.sort = pick(hf.filters.sort, ['downloads', 'likes', 'last_modified', 'created_at', 'trending_score'] as const, 'downloads'); hf.filters.direction = pick(String(hf.filters.direction), ['-1', '1'] as const, '-1'); hf.filters.limit = clamp(hf.filters.limit, 50, 1, 200, true); hf.filters.includePrivate = bool(hf.filters.includePrivate, true); hf.filters.onlyChatCompatible = bool(hf.filters.onlyChatCompatible, true); hf.filters.provider = str(hf.filters.provider, '', false).trim();
