...
function normNodes(v: unknown, warnings: string[]): ApiNode[] { const src = Array.isArray(v) ? v : DEFAULT_CONFIG.llm.apiKeys; const out = src.filter(isObj).map((n, i) => ({ name: str(n.name, `节点 ${i + 1}`, false), baseUrl: str(n.baseUrl, ''), key: str(n.key, ''), enabled: bool(n.enabled, true), models: list(n.models), modelsFetchedAt: str(n.modelsFetchedAt, '') })); if (!out.length) { warnings.push('llm.apiKeys 为空或格式错误，已补入默认节点。'); return clone(DEFAULT_CONFIG.llm.apiKeys); } return out; }
