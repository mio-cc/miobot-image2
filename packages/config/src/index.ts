...
export interface ApiNode { name: string; baseUrl: string; key: string; enabled: boolean; models: string[]; modelsFetchedAt: string; provider?: string; headers?: Record<string, string> }
...
const DEFAULT_CONFIG: AppConfig = {
  ...
  napcat: { wsUrl: 'ws://localhost:3001', token: '', mountOutputDir: '', actionTimeoutMs: 15000, textSendTimeoutMs: 15000, imageSendTimeoutMs: 120000, fileSendTimeoutMs: 300000, fileSendRetryCount: 1, fileSendRetryDelayMs: 1200, forwardSendTimeoutMs: 300000, getMessageTimeoutMs: 10000 },
...
function normalizeAppConfig(input: R, warnings: string[]): AppConfig {
  ...
  const canvas = { ...DEFAULT_CONFIG.canvas, ...obj((cfg as any).canvas), brand: { ...DEFAULT_CONFIG.canvas.brand, ...obj((cfg as any).canvas?.brand) }, logs: { ...DEFAULT_CONFIG.canvas.logs, ...obj((cfg as any).canvas?.logs) } }; canvas.imageNodeIndex = clampNode(canvas.imageNodeIndex, llm.imageNodeIndex); canvas.editNodeIndex = clampNode(canvas.editNodeIndex, llm.editNodeIndex); canvas.interrogateNodeIndex = clampNode(canvas.interrogateNodeIndex, llm.interrogateNodeIndex); canvas.interrogateTemplateNodeIndex = clampNode(canvas.interrogateTemplateNodeIndex, canvas.interrogateNodeIndex); canvas.imageEditRequestMode = pick(canvas.imageEditRequestMode, ['auto', 'json-images', 'json-image', 'multipart'] as const, 'auto'); canvas.defaultQuality = imageQuality(canvas.defaultQuality, 'auto'); canvas.defaultOutputFormat = pick(canvas.defaultOutputFormat, ['png', 'jpeg', 'webp'] as const, 'png'); canvas.defaultCount = pick(num(canvas.defaultCount), [1, 2, 4, 8, 16] as const, 1); canvas.imageTimeoutMs = clamp(canvas.imageTimeoutMs, llm.imageTimeoutMs, 30000); canvas.interrogateTimeoutMs = clamp(canvas.interrogateTimeoutMs, llm.interrogateTimeoutMs, 30000); canvas.interrogateTemplateTimeoutMs = clamp(canvas.interrogateTemplateTimeoutMs, canvas.interrogateTimeoutMs, 30000); canvas.brand.logoText = str(canvas.brand.logoText, DEFAULT_CONFIG.canvas.brand.logoText, false).trim() || DEFAULT_CONFIG.canvas.brand.logoText; canvas.brand.pageTitle = str(canvas.brand.pageTitle, DEFAULT_CONFIG.canvas.brand.pageTitle, false).trim() || DEFAULT_CONFIG.canvas.brand.pageTitle; canvas.brand.faviconUrl = str(canvas.brand.faviconUrl, DEFAULT_CONFIG.canvas.brand.faviconUrl, false).trim() || DEFAULT_CONFIG.canvas.brand.faviconUrl; canvas.logs.level = pick(canvas.logs.level, ['debug', 'info', 'warn', 'error'] as const, 'info'); canvas.logs.maxMemoryEntries = clamp(canvas.logs.maxMemoryEntries, 1000, 100, 5000, true); canvas.maxHistory = clamp(canvas.maxHistory, 50, 1, 500, true); canvas.dataDir = str(canvas.dataDir, '', false).trim();
  ...
  return { panel: { port: clamp(panel.port, 3018, 1, 65535, true), passwordSeed: str(panel.passwordSeed, 'change-me-on-first-login', false) }, napcat: { wsUrl: str(nap.wsUrl, DEFAULT_CONFIG.napcat.wsUrl), token: str(nap.token, ''), mountOutputDir: str(nap.mountOutputDir, ''), actionTimeoutMs: clamp(nap.actionTimeoutMs, 15000, 3000), textSendTimeoutMs: clamp(nap.textSendTimeoutMs, 15000, 3000), imageSendTimeoutMs: clamp(nap.imageSendTimeoutMs, 120000, 10000), fileSendTimeoutMs: clamp(nap.fileSendTimeoutMs, nap.forwardSendTimeoutMs || 300000, 10000), fileSendRetryCount: clamp(nap.fileSendRetryCount, 1, 0, 10, true), fileSendRetryDelayMs: clamp(nap.fileSendRetryDelayMs, 1200, 0, 30000, true), forwardSendTimeoutMs: clamp(nap.forwardSendTimeoutMs, 300000, 10000), getMessageTimeoutMs: clamp(nap.getMessageTimeoutMs, 10000, 3000) }, promptsChat: pc, canvas, freeMode: free, huggingFace: hf, llm, bot };
}
...
function normHeaders(v: unknown): Record<string, string> { const source = obj(v); const out: Record<string, string> = {}; for (const [key, value] of Object.entries(source)) { const name = String(key || '').trim(); const text = String(value ?? '').trim(); if (name && text) out[name] = text; } return out; }
function normTtsVoiceIds(v: unknown, active: string): string[] { const source = Array.isArray(v) ? v : typeof v === 'string' ? v.split(/[,\uFF0C\u3001\uFF1B;\n]+/) : []; const seen = new Set<string>(); const out: string[] = []; for (const x of [active, ...source]) { const s = String(x ?? '').trim(); if (s && !seen.has(s)) { seen.add(s); out.push(s); } } return out; }
